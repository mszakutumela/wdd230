...
function displayLinks(weeks) {
    weeks.foreach((week)=>{
        ...
        //W04
        const title4 = week.links[4].title;
        const url = week.links[4].url;

        if (title4 == "" && url4 == "") {activities.innerHTML = `${weekNumber}: <a href="${url1}">${title1}</a> | <a href="${url2}">${title2}</a> | <a href="${url3}">${title3}</a>`;
                    }
                    else {
